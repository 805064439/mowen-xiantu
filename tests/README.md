...
三层结构：**代码实现 → 已部署产物** 全覆盖，共 455 项用例。

| 层 | 目录 | 工具 | 数量 | 测什么 |
|---|---|---|---|---|
| 后端单元 | `tests/backend` | pytest | 387 | 天道引擎的全部纯逻辑：数值/判定/经济/状态清洗/NPC/接口契约/修炼节奏（含高风险波动、闭门打扰） |
| 前端单元 | `tests/frontend` | vitest + happy-dom | 68 | 存档持久化、仙缘令编解码、前后端常量一致性（含修炼系数/连击/闭门/风险波动同源比对） |
