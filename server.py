...
# v3.12：追索的终点是「照面」，不是「下落已明」。
# 旧文案写「再往下只有同一个下落……算是有了交代，转身去顾别的事」——
# 玩家读到的还是「没见到人」，追了五轮换一句安慰，比不收场更像敷衍。
# 玩家追人就是要追到这张脸，代码兜底也必须把人送到眼前。
STALL_TAKEOVER_TEXT = (
...
# AI 图省事时最爱这一手（一场坟、一块无字木牌就把九轮的追索打发了），
# 玩家连打了这么久，等来的是「死讯」，比一直找不到更让人泄气。
# 玩家要的是找着人，所以收场句一律不许沾这些词——命中就退回中性的照面兜底。
STALL_DEAD_END_WORDS = (
    "死", "亡", "殁", "故去", "已故", "身故", "过世", "辞世", "下世",
    "坟", "冢", "墓", "牌位", "灵位", "白骨", "尸",
    "失踪", "下落不明", "查无此人", "不知所踪", "杳无音信", "音讯断绝",
    # v3.12.2：线上验证抓到的漏网之鱼——「菱姑已三年无音信」被当成有效线索
    # 摘进了收场句，于是同一句话里「当面见了」跟「无音信」并排出现。
    # 「有/无音信」这一族要成对堵：只堵「杳无音信」，AI 换个说法就绕过去了。
    "无音信", "音信全无", "无消息", "杳无消息", "无踪迹", "不见踪影",
    "未见踪影", "踪影全无", "再无消息", "绝了音信",
    "此事作罢", "就此划去", "不必再追", "再无线索",
)
# 收场要给的是**人**，不是下落。{} 处填最近一条线索留下的去向（由 chronicle 末条摘出，
# 且必须先把上面的死讯词滤掉——否则这句会变成「人死了」的通知书）。
STALL_TAKEOVER_LINES = (
    "几番周折，人终是寻着了——{}",
    "路走到尽头，人就在那里——{}",
    "这一趟没有落空，当面见了——{}",
)
STALL_TAKEOVER_LINE_FALLBACK = "人已寻着，当面说个明白"
# 接管后线索留在册上的标记：注销等于「这件事没了」，而玩家追的是人，
# 人找着了正该是故事的开始。留着它，下一轮的【未决之事】才有一条可续的话头。
STALL_FOUND_NOTE = "人已寻着，当面在此"
...
def top_chase_thread(state: dict, prev_tid: int = 0) -> dict | None:
    ...
    """
    # found = 已经照过面的线（代码收场把人送到眼前）。它还在册上等着续话，
    # 但不再是「还在追的人」——否则下一轮就会被当成新一轮追索，再追五轮。
    pool = [t for t in (state.get("threads") or [])
            if _to_int(t.get("chase"), 0) > 0 and not _to_int(t.get("found"), 0)]
    if not pool:
        return None
...
def last_clue_line(state: dict) -> str:
    """摘出最近一条线索留下的去向，给收场文案用——玩家要的是人，不是「算了」。

    摘到的句子若沾了死讯 / 失踪一类词（一张坟、一块无字木牌就把追索打发了），
    一律作无效处理，好让调用方退回中性照面句——那种话不该由代码替 AI 说出来。
    """
    ...
    for sep in ("：", ":"):
        if sep in tail:
            ...
            if body and not is_dead_end_line(body):
                return body[:THREAD_NOTE_MAX]
    return ""


def is_dead_end_line(text: str) -> bool:
    """这句话是不是在告诉玩家「人没了 / 找不着了」。

    2026-09-23 的日志里，AI 用「柳三娘殁于腊月十九」加一块无字木牌了结了九轮追索，
    玩家读起来就是人被写死了。追人追到这个结果，比一直找不到更让人泄气。
    """
    s = str(text or "")
    return any(w in s for w in STALL_DEAD_END_WORDS)
...
def stall_prompt_note(state: dict) -> str | None:
    ...
    if hop >= 2:
        ...
        head += (
            ...
            f"目标始终在前一站。玩家追的是这个人，不是车票。\n"
            "本轮**不许再写「已不在、去了别处、指向下一个地方」**，也不许用一句"
            "「无从查考、就此断了」收尾：把人写出来——当面说上话也好、擦肩被认出也好、"
            "隔街遥见也好。若这一处确实不该相遇，那就让他在玩家眼前路过、"
            "或亲自差人递一句话来。**目标是让玩家看见这个人本人**，不是再听一句去向。\n"
        )
    body = ""
    if n >= STALL_FORCE_TURNS:
        # v3.12：照面从「首选」升为**唯一出路**。旧版把「换来确凿情报」与「办成」并列，
        # AI 于是专挑最好写的那一项——每轮给一个新地名，条条通往下一站；再逼得紧些，
        # 它就用一场坟、一块无字木牌把人写死了事。玩家追九轮等来一纸死讯，比找不到更泄气。
        # 追人就只有一个结果：人站在面前。退路一概撤掉。
        body = (
            f"这是玩家第 {n} 轮在追同一件事（{label}），前几轮都没见着人。"
            "本轮**必须让这个人出场**，没有别的选项。\n"
            "玩家追这么多轮，想看的就是这张脸。任选一种写法，写清在何处、以何种方式见着，"
            "以及这一面的结果（说开了 / 不愿相认 / 另有后话）：\n"
            "· 当面说上话；\n· 隔街遥见，隔着人望了一眼；\n· 擦肩错身而被认出；\n"
            "· 本人当面递来亲笔与信物。\n"
            "**以下收尾一概严禁**：\n"
            "· 人死了、已经过世、只剩一座坟一块牌位——最省事的写法，也最扫兴；\n"
            "· 失踪了、下落不明、查无此人、音讯断绝、无从查考；\n"
            "· 「一无所获、线索又断了」「再无线索可寻」「此事作罢」；\n"
            "· 只由旁人转述一句「他往某处去了」——那已是第 "
            f"{n} 次让玩家空手而归，属于叙事失败。\n"
            "若本轮还是没让人出场，天道会亲自把这个人送到玩家面前——"
            "追索到此为止，接下去是当面打交道。"
        )
    return head + body
...
def takeover_choices(choices: list, label: str, action_text: str) -> list:
    ...
    旧版三条是「收心 / 收心 / 改道」——正文写着人就在眼前，选项里却没有一个字
    跟这个人有关，玩家只能自由输入才能搭话。2026-09-24 日志里，玩家就是这样
    连着几轮免费 Toxic不见人的：找到了也等于没找到。

    前两条里凡仍在追本线的一律丢掉，从 STALL_TAKEOVER_SWAPS 按序补齐一条；
    最后一条固定是改道出口（与 ensure_resolve_choice 的措辞同源，玩家容易认）。
    """
    kept: list = []
...
def sanitize_threads(raw: Any) -> list:
    ...
    for t in raw[:THREAD_MAX]:
        ...
        out.append({
            ...
            "found": 1 if _to_int(t.get("found"), 0) else 0,     # 已照面：人找着了，余下是续话
        })
    return out
...
19. 若收到【追索已滞】：玩家已连追同一件事数轮仍没见着人，本轮**必须让这个人出场**——
    当面说上话 / 隔街遥见 / 擦肩被认出 / 本人当面递物，任选一种，并写明在何处、
    以何种方式见着、这一面的结果。**没有退路**：不许写成「人已故去/只剩一座坟」，
    不许写成「失踪、下落不明、查无此人、音讯断绝」，也不许只让旁人转述一句
    「他往某处去了」就交差。再拖，天道会亲自把这个人送到玩家面前，届时由不得 AI。
...
def _postprocess_turn(state: dict, data: dict, meta: dict, action_text: str,
    ...
    if took_over:
        ...
        # 旧做法是 remove(hit)——注销这条线。那等于告诉玩家「这件事没了」，
        # 而玩家追的是人：人找着了才是故事的开始。故转成「当面在此」留在册上，
        # 下一轮的【未决之事】里才有一条可续的话头（并且打上 found，不再计入追索）。
        hit = find_thread(state.get("threads") or [], label)
        if hit is not None:
            hit["note"] = STALL_FOUND_NOTE
            hit["found"] = 1
            hit["chase"] = 0
            hit["last"] = turn_now
