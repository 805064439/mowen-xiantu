...
function submit() {
  ...
  if (!t || game.loading || game.ended) return;
  text.value = "";
...
<template>
  <div id="custom-input">
    <input id="free-text" ref="inputEl" v-model="text" list="custom-history" maxlength="60"
           ...
           :disabled="game.inputLocked || game.ended" @keydown="onKeydown">
    <datalist id="custom-history">
      <option v-for="h in history" :key="h" :value="h"></option>
    </datalist>
    <button id="btn-custom" type="button" :disabled="game.inputLocked || game.ended" @click="submit">行 事</button>
  </div>
</template>
